...
abstract class MyStriped64 extends Number {
    ...
    static {
        System.out.println("Runtime.getRuntime().availableProcessors():"+NCPU);
    }
            ...
            if ((as = cells) != null && (n = as.length) > 0) {
                ...
                if ((a = as[(n - 1) & h]) == null) { //下标计算：(n - 1) & h   为空就走创建逻辑
                    if (cellsBusy == 0) {       // Try to attach new Cell
                        ...
                        if (cellsBusy == 0 && casCellsBusy()) {  //标记cellBusy
                            boolean created = false; //已创建标识
...
//                    collide = false;
                    ...
                    System.out.println("pass"); //这样就不会再走到下面的扩容else if了
